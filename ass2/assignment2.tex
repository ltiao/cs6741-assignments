...
Now as we have exhausted all cases, then any \textsc{Yes} instance of \textsc{Independent Set} corresponds to a \textsc{Yes} instance of $d-$\textsc{Packing} for any fixed $d$. Since $k' = k$ then it is upper bounded by the identity function. The running time of the algorithm is polynomial as $d$ is fixed. Hence we have the reduction 
