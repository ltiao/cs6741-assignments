...
(n.b. $N_G^i[v]$ denotes the closed $i$th neighborhood of $v$, that is, $u \in N_G^i[v]$ iff $d(u, v) \leq i$.) The table for $p(X, d)$ is filled by increasing cardinalities of $X$ and therefore we compute it $\mcO^*(2^n)$ time and space. 
...
Consider the algorithm from (1) (based on the algorithm by \cite{Wahlstr2008,Dahllof2005}) which can count the number of $d$-packings in a given graph $G$ and integer $d \ge 0$ in $\mcO^*(1.2377^n)$ time and polynomial-space. Then, we obtain \todo{fill in more details here} a polynomial-space algorithm for \textsc{Packing Coloring} with running time $\mcO^*(2.2377^n)$.
