...
For even $d=2m$, we first add $m$ vertices for all pairs of adjacent vertices as before. For edge $i$ denote its added vertices $V_{i} = \{w_{i1}, w_{i2}, \dotsc, w_{im}\}$ for $i \in \{1, 2, \dotsc, |E|\}$ and let $V^{*} = \bigcup_{i=1}^{|E|} V_i, V' = V \cup V^{*}$. Then for each pair of vertices $(u, v) \in V_i \times V_j$ where $i \ne j$, we add a path graph on $2m-1 = d-1$ vertices. In other words, we add a path with $d-1$ vertices between vertices $w_{ip}$ and $w_{jq}$ where $i\ne j$.
...
\begin{itemize}
	\item ($\Rightarrow$) Let $S$ be an independent set of size $k$ in $G$. Then, for odd $d$, the distance between any two distinct vertices in $S$ in $G'$ is at least $2(m+1)=d+1$ since 

	, whereas in even-$d$ case, if two vertices were originally not neighbours, they are now of distance $d-1 + 2 = d+1$ apart. Hence, $S$ is a $d$-\textsc{Packing} of $G'$ since the distance between any pair of the vertices of $S$ in $G'$ is at least $d+1$.
	\item ($\Leftarrow$) 
\end{itemize}
